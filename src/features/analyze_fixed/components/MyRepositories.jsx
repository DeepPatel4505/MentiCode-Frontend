...
function MyRepositories() {
    ...
    const isGithubUser = Boolean(user?.githubId) || (user?.loginProvider || "").toLowerCase() === "github";
    ...
    const fetchRepos = async () => {
        ...
        try {
            ...
            setError(fetchError?.message || "Failed to fetch GitHub repositories.");
            setRepoSource([]);
        } finally {
            setIsLoading(false);
        }
    ...
    const handleSendToPlayground = async (repository) => {
        try {
            setSendingRepoId(repository.id);
            setError("");

            const createdPlayground = await sendGithubRepoToPlayground({
                owner: repository.owner,
                repoName: repository.name,
                defaultBranch: repository.defaultBranch,
            });

            navigate(`/analyze/playground/${createdPlayground.id}`);
        } catch (sendError) {
            setError(sendError?.message || "Failed to import repository into playground.");
        } finally {
            setSendingRepoId("");
        }
    };
    ...
    return (
        <section className="w-full" aria-label="My Repositories">
            ...
            {!isGithubUser && (
                <div >
                    <p >
                        Repository analysis requires a GitHub-connected account.
                    </p>
                    <Link
                        to="/profile"
                                ...
                                >
                                    <span className="flex items-center gap-4 min-w-0 flex-1">
                                        ...
                                        <span
                                            ...
                                            aria-hidden="true"
                                        >
                                            {languageToSymbolMap[(repository.primaryLanguage || "").toLowerCase()] || "--"}
                                        </span>
                                    ...
                                    >
                                        ...
                                        <button
                                            ...
                                            className="
                                                ...
                                                shadow-[0_2px_8px_rgba(0,0,0,0.1)]
                                            "
                                            onClick={() => handleSendToPlayground(repository)}
                                        >
                                            Send to Playground
                                        </button>
                                    </div>
