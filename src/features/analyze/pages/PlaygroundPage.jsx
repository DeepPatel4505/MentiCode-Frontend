...
function PlaygroundPage() {
            ...
            setPlaygrounds(prev =>
                prev.map(pg => {
                    ...
                    const mostUsed = keys.length
                        ? keys.reduce((a, b) => languageCount[a] > languageCount[b] ? a : b)
                        : null;
                    return {
                        ...pg,
                        language: mostUsed
                            ? (languageToSymbolMap[mostUsed.toLowerCase()] ?? mostUsed.toUpperCase())
                            : "--",
                    };
                })
            );
    ...
    const handleConfirmDelete = async () => {
        ...
        try {
            ...
            setPlaygrounds(prev => prev.filter(pg => pg.id !== deleteConfirmation.id));
            setActiveCount(prev => prev - 1);
            setNotification({ type: "success", message: `"${deleteConfirmation.name}" deleted` });
    ...
    return (
        <section className="w-full max-w-6xl mx-auto pb-12">
            ...
            <div className="flex items-start justify-between mb-8">
                <div>
                    <h1 className="text-xl font-semibold text-white tracking-tight">My Playgrounds</h1>
                    <p className="mt-1 text-sm text-neutral-500">Manage and open your code analysis sessions.</p>
                </div>
                ...
                <div className="flex flex-col items-end gap-1.5">
                    <div className="flex items-center gap-2 text-xs">
                        ...
                        <span className={cn("font-semibold", isLimitReached ? "text-red-400" : "text-neutral-300")}>
                            {activeCount}/{PLAYGROUND_LIMIT}
                        </span>
                    </div>
                    <div className="w-28 h-1 bg-neutral-800 rounded-full overflow-hidden">
                        <div
                            className={cn("h-full rounded-full transition-all duration-300", isLimitReached ? "bg-red-500" : "bg-violet-500")}
                            style={{ width: `${(activeCount / PLAYGROUND_LIMIT) * 100}%` }}
                        />
                    </div>
                </div>
            </div>
                    ...
                    >
                        ...
                        <div className="px-4 pt-4 pb-3 flex items-start justify-between gap-2 shrink-0">
                            ...
                            <div className="relative shrink-0">
                                <button
                                    type="button"
                                    onClick={() => setActiveMenu(activeMenu === playground.id ? null : playground.id)}
                                ...
                                <CardOptionsMenu
                                    ...
                                    isLoading={isDeleting && deleteConfirmation?.id === playground.id}
                                />
                            </div>
                        </div>

                        {/* Preview area */}
                        <div className="flex-1 mx-4 mb-3 rounded-md bg-neutral-950/60 border border-neutral-800 flex items-center justify-center">
                            <span className="text-xs text-neutral-700 font-mono">preview</span>
                        </div>
                        ...
                        <div className="px-4 pb-4 shrink-0">
                            <button
                                type="button"
                                onClick={() => handleEnterPlayground(playground.id)}
            ...
            {playgrounds.length === 0 && (
                <div className="mt-8 text-center py-12">
                    <p className="text-sm text-neutral-500">No playgrounds yet.</p>
                    <p className="text-xs text-neutral-700 mt-1">Click "New Playground" to create one.</p>
                </div>
            )}
